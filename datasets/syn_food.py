...
class Syn_Food(DatasetBase):
    ...
    def __init__(self, root, num_shots, subsample, dalle=True):
        ...
        if not dalle:
            self.image_dir = '/data/mushui/dataset/StableDiffusion/food-101'
            self.split_path = os.path.join(self.dataset_dir, 'stable_diffusion_food.json')
