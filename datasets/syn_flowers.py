...
class Syn_Flowers(DatasetBase):
    
    dataset_dir = 'dalle_flower'
    ...
    def __init__(self, root, num_shots, subsample, dalle=True):
        ...
        if not dalle:
            self.image_dir = '/data/mushui/dataset/StableDiffusion/oxford_flowers'
            self.split_path = os.path.join(self.dataset_dir, 'stable_diffusion_flowers.json')
