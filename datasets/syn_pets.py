...
class Syn_Pets(DatasetBase):
    
    dataset_dir = 'dalle_pets'
    ...
    def __init__(self, root, num_shots, subsample, dalle=True):
        ...
        if not dalle:
            self.image_dir = '/data/mushui/dataset/StableDiffusion/oxford_pets'
            self.split_path = os.path.join(self.dataset_dir, 'stable_diffusion_pets.json')
