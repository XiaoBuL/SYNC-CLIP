...
class Syn_UCF(DatasetBase):
    ...
    def __init__(self, root, num_shots, subsample, dalle=True):
        ...
        if not dalle:
            self.image_dir = '/data/mushui/dataset/StableDiffusion/ucf101'
            self.split_path = os.path.join(self.dataset_dir, 'stable_diffusion_ucf.json')
