...
class Syn_fgvc(DatasetBase):
    
    dataset_dir = 'dalle_fgvc'
    ...
    def __init__(self, root, num_shots, subsample, dalle=True):
        ...
        if not dalle:
            self.image_dir = '/data/mushui/dataset/StableDiffusion/fgvc_aircraft'
            self.split_path = os.path.join(self.dataset_dir, 'stable_diffusion_fgvc.json')
