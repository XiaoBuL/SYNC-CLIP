...
class ResidualAttentionBlock_SynCLIP(nn.Module):
    ...
    def forward(self, input):
        ...
        if self.add_prompt:
            ...
            if not self.text_layer:
                ...
                if syn_prompt is None:
                    visual_context = self.VPT_shallow.expand(x.shape[1], -1, -1).permute(1, 0, 2).half()
                else:
                    visual_context = self.VPT_shallow.expand(x.shape[1], -1, -1).permute(1, 0, 2).half()
                    # visual_context = self.syn_projection(visual_context)
                    
                    proj_syn = False
                    if proj_syn:                    
                        visual_context = self.syn_projection(visual_context)                
                    else:      
                        visual_context_syn = syn_prompt[0].expand(x.shape[1], -1, -1).permute(1, 0, 2).half()
                        visual_context = torch.cat([visual_context_syn, visual_context[visual_context_syn.shape[0]:]], dim=0)
                    syn_prompt = syn_prompt[1:]
...
class Transformer(nn.Module):
    def __init__(self, width: int, layers: int, heads: int, attn_mask: torch.Tensor = None, prompts_needed=0,
        ...
        self.resblocks = nn.Sequential(*[ResidualAttentionBlock_SynCLIP(width, heads, attn_mask, True,
                                                                        text_layer, i,
                                                                        design_details) if prompts_needed > i
                                            else ResidualAttentionBlock_SynCLIP(width, heads, attn_mask, False,
                                                                            text_layer, i, design_details)
                                            for i in range(layers)])

    def forward(self, input):
        return self.resblocks(input)
...
class VisionTransformer(nn.Module):
    def __init__(self, input_resolution: int, patch_size: int, width: int, layers: int, heads: int,
        ...
        if self.VPT_shallow:
            ...
            n_ctx_syn = design_details["vision_ctx_syn"]  # hyperparameter
            
            ctx_vectors = torch.empty(n_ctx, width)
            ctx_vectors_syn = torch.empty(n_ctx_syn, width)
            
            nn.init.normal_(ctx_vectors, std=0.02)
            nn.init.normal_(ctx_vectors_syn, std=0.02)
            
            self.VPT = nn.Parameter(ctx_vectors)
            self.SYN_VPT = nn.Parameter(ctx_vectors_syn)            
    ...
    def forward(self, x: torch.Tensor, syn_prompt=None):
        ...
        if self.VPT_shallow:
            if syn_prompt is not None:
                proj_syn = False
                visual_ctx = self.VPT.expand(x.shape[0], -1, -1).half()
                if proj_syn:
                    visual_ctx = self.syn_projection(visual_ctx)
                else:            
                    visual_ctx_syn = self.SYN_VPT.expand(x.shape[0], -1, -1).half()
                    visual_ctx = torch.cat([visual_ctx_syn, visual_ctx[:, visual_ctx_syn.shape[1]:]], dim=1)
            else:            
                visual_ctx = self.VPT.expand(x.shape[0], -1, -1).half()
        ...
        x = self.transformer([x, syn_prompt])[0]
